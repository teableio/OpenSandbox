...
| `execd_delivery` | string | `"init_container"` | How execd reaches the sandbox pod. **`init_container`** copies it from `runtime.execd_image` into an emptyDir. **`image_volume`** mounts `runtime.execd_image` read-only as a Kubernetes [image volume](https://kubernetes.io/docs/tasks/configure-pod-container/image-volumes/) (GA in Kubernetes 1.35; needs a CRI with image volume support such as containerd ≥ 2.1), skipping the init container lifecycle. Windows profiles and `egress.disable_ipv6` always keep the init container. |
