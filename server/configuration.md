...
| `execd_delivery` | string | `"init_container"` | How execd reaches the sandbox pod. **`init_container`** copies it from `runtime.execd_image` into an emptyDir. **`image_volume`** mounts `runtime.execd_image` read-only as a Kubernetes [image volume](https://kubernetes.io/docs/tasks/configure-pod-container/image-volumes/) (beta and enabled by default in Kubernetes 1.35, GA in 1.36; the `ImageVolume` feature gate is off by default in 1.33–1.34; needs a CRI with image volume support such as containerd ≥ 2.1), skipping the init container lifecycle. `pullPolicy` is left to the Kubernetes default (`Always` for `:latest`, otherwise `IfNotPresent`), matching the init container. Windows profiles and `egress.disable_ipv6` always keep the init container. |
